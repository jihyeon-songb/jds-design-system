...
- Apply it only to `jds-` class namespace elements; do not style `:root`, `html`, `body`, or consumer content.
...
- Produces: `--jds-typography-body-font-size: 14px;` and a component-scoped `font-size: var(--jds-typography-body-font-size);` rule.
...
- [ ] **Step 3: Add the token and one scoped component rule**
  ...
  Add this group to `packages/tokens/src/jds.tokens.json`:
  ...
  [class^="jds-"],
  [class*=" jds-"] {
    font-size: var(--jds-typography-body-font-size);
  }
  ```
...
- [ ] **Step 4: Run focused verification**
  ...
  Run: `pnpm vitest run packages/tokens/tests/build.test.ts && rg -n -- '--jds-typography-body-font-size: 14px' packages/tokens/dist/tokens.css && rg -n 'font-size: var\(--jds-typography-body-font-size\)' packages/components/src/index.css`

  Expected: PASS; the generated CSS exports 14px and the single JDS-scoped
  rule consumes it.
...
- [ ] **Step 6: Commit the typography change**
  ...
  ```bash
  git add packages/tokens/src/jds.tokens.json packages/tokens/tests/build.test.ts packages/tokens/dist/index.d.ts packages/tokens/dist/index.js packages/tokens/dist/tokens.css packages/components/src/index.css
